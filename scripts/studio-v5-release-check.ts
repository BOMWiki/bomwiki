...
const manifest = {
  ...
  checks, evidence: evidenceManifests,
  gates: {
    ...
    visualEvidence: visual?.status || 'missing', humanVisualReview: visual?.humanReview?.status || 'pending',
    liveDeployment: 'not-run-by-local-release-check', finalReleaseSignoff: 'blocked-until-human-review-and-live-verification',
  },
  status: automatedChecksPass && evidenceValid ? 'automated-candidate-pass-human-review-required' : 'failed',
};
...
if (!automatedChecksPass || !evidenceValid) process.exitCode = 1;
